    ...
    public static Line getMasterOutputLine() {
        for (Mixer mixer : getMixers()) {
            for (Line line : getAvailableOutputLines(mixer)) {
               //if (line.getLineInfo().toString().contains("Ljavax.sound.sampled.Mixer$Info;@45820e51")) return line;
               if (line.getLineInfo().toString().contains("SPEAKER target port")) return line;
